...
use app\BaseService;
use TAnt\App\Model\Role;
use TAnt\App\Model\User;
use TAnt\App\Model\Permission;
use TAnt\Contract\InterfaceService;

class UserService extends BaseService implements InterfaceService
{
    public $model;
    ...
    {
        $user = $this->model->findByName($username);
        if (empty($user)) {
            return false;
        }
     ...
     * @return array
     */
    public function formatRoute(array $data)
    {
        $routes = [];
    ...
    {
        $map = [];
        if (isset($params['deptId']) && !empty($params['deptId'])) {
            $map['u.dept_id'] = $params['deptId'];
        }
    ...
    public function create(array $data)
    {
        return $this->transaction(function () use ($data) {
            ...
            //绑定角色
            $this->bindRole($user, $data['roles']);

            if (!empty($data['posts'])) {
                //绑定岗位
                $this->bindPost($user, $data['posts']);
            }
    ...
    {
        $user->posts()->detach($user->posts->column('id'));
    }
